...
with col_f2:
    sort_by = st.selectbox("Sort by", ['max_risk', 'mean_risk', 'months'], format_func=lambda x: {
        'max_risk': 'Peak Risk', 'mean_risk': 'Avg Risk', 'months': 'Months Tracked'
    }[x])
with col_f3:
    min_risk = st.slider("Min. Peak Risk", 0.0, 1.0, 0.0, 0.05)

filtered = fleet[
    fleet['risk_level'].isin(risk_filter) &
    (fleet['max_risk'] >= min_risk)
].sort_values(sort_by, ascending=False)

st.markdown(f"<p style='color:rgba(255,255,255,0.4); font-size:13px;'>Showing {len(filtered)} of {len(fleet)} aircraft</p>", unsafe_allow_html=True)
...
fig_scatter = px.scatter(
    filtered,
    x='mean_risk', y='max_risk',
    color='risk_level',
    color_discrete_map=color_map,
    hover_name='aircraft_id',
    hover_data={'mean_risk': ':.3f', 'max_risk': ':.3f', 'months': True, 'risk_level': False},
    size='months',
    size_max=18,
...
def highlight_risk(val):
    if val == 'High':
        ...
        return 'color: #ffaa00; font-weight: 700'
    return 'color: #00cc77; font-weight: 700'

styled = display[['Aircraft ID', 'Peak Risk', 'Avg Risk', 'Risk Level', 'Months', 'Last Record']].style.applymap(
...
# ─── Monthly heatmap ──────────────────────────────────────────────────────────
st.markdown("---")
st.markdown("**Seasonal Risk Patterns — Monthly Average**")
...
fig_heat = go.Figure(go.Heatmap(
    ...
    showscale=True,
    colorbar=dict(tickfont=dict(color='white'), title=dict(text='Risk', font=dict(color='white'))),
))
